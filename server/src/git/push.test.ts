...
describe("push", () => {
  ...
  it("overwrites the remote when forced", async () => {
    await setupAmended();
    const res = await push(WORK, { force: true });
    expect(res.rejected).toBeUndefined();
    // Origin now points at the amended local commit.
    expect(await revParse(ORIGIN, "main")).toBe(await revParse(WORK, "HEAD"));
  });

  it("refuses even a force push when the remote advanced unseen (protects unpulled work)", async () => {
    // work1 pushes; a second clone advances origin; work1 commits without
    // fetching → histories diverge because the REMOTE moved.
    await fs.mkdir(BASE, { recursive: true });
    await runGit(BASE, ["init", "--bare", "origin.git"]);
    await fs.mkdir(WORK, { recursive: true });
    await runGit(WORK, ["init", "-b", "main"]);
    ...
    await expect(push(WORK, { force: true })).rejects.toMatchObject({ status: 409 });
    expect(await revParse(ORIGIN, "main")).toBe(theirHead);
  });

  it("pushes cleanly when the branch is a fast-forward", async () => {
    await setupAmended();
    await push(WORK, { force: true }); // sync origin to local
    await fs.writeFile(path.join(WORK, "a.txt"), "one\ntwo\nthree\n", "utf8");
